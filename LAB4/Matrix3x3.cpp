...
Vector3D Matrix3x3::operator*(const Vector3D& aVector) const noexcept
{
	...
	return Vector3D(M[0].dot(aVector),
		M[1].dot(aVector),
		M[2].dot(aVector));
}
...
{
	float radA = aAngleInDegree * std::numbers::pi_v<float> / 180.0f;
	float cosA = std::cos(radA);
	float sinA = std::sin(radA);

	return Matrix3x3(Vector3D(cosA, -sinA, 0.0f),
		Vector3D(sinA, cosA, 0.0f),
		Vector3D(0.0f, 0.0f, 1.0f));
}
